...
export default function VideoSection() {
  ...
  const archives = (galerieItems ?? []).filter(
    (i) => i.type === "video" && i.categorie === "Divers" && i.videosExtras && i.videosExtras.length > 0
  ) as ArchiveItem[];

  function ouvrir(spec: ArchiveItem) {
    const firstId = spec.videosExtras?.[0]?.id ?? getYoutubeId(spec.urlVideo ?? "") ?? "";
    setModal({ spec, videoId: firstId });
  }
  ...
  return (
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: 64 }}>
        {archives.map((spec, i) => {
          const thumbnail = spec.videosExtras?.[0]?.id ?? getYoutubeId(spec.urlVideo ?? "") ?? "";
          return (
            <button
              key={spec._id}
              onClick={() => ouvrir(spec)}
            ...
            >
              ...
              <div style={{ position: "relative", aspectRatio: "16/9", overflow: "hidden", borderBottom: "1px solid var(--ink-line)", background: ACCENT_WASHES[i % 4] }}>
                ...
                <div style={{
                  ...
                  background: "rgba(42,39,34,0.18)", transition: "background 140ms",
                }}>
                  <div style={{
                    width: 44, height: 44, borderRadius: "50%",
                    background: "rgba(245,239,227,0.92)",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: "1.125rem", paddingLeft: 3,
                    boxShadow: "0 2px 8px rgba(0,0,0,0.2)",
                  }}>▶</div>
                </div>
                {/* Nombre de vidéos */}
                {(spec.videosExtras?.length ?? 0) > 1 && (
                  <div style={{
                    position: "absolute", bottom: 8, right: 8,
                    background: "rgba(42,39,34,0.75)", color: "#F5EFE3",
                    fontFamily: "var(--font-worksans)", fontSize: "0.6875rem", fontWeight: 600,
                    padding: "2px 8px", borderRadius: 4,
                  }}>
                    {spec.videosExtras!.length} vidéos
                  </div>
                )}
              </div>
      ...
      <AnimatePresence>
        {modal && (
          <motion.div
            ...
            onClick={fermer}
            style={{
              position: "fixed", inset: 0, background: "rgba(42,39,34,0.7)",
          ...
          >
            <motion.div
              ...
              style={{
                background: "#FBF7EC", border: "1px solid var(--ink-line)", borderRadius: 8,
                width: "100%", maxWidth: 860,
                boxShadow: "8px 8px 0 rgba(42,39,34,0.12)", overflow: "hidden",
              }}
            >
              {/* En-tête modal */}
              <div style={{
                display: "flex", justifyContent: "space-between", alignItems: "center",
                padding: "16px 20px", borderBottom: "1px solid var(--ink-line)",
              }}>
                <div>
                ...
                <button
                  ...
                  aria-label="Fermer"
                  style={{
                    background: "transparent", border: "1px solid var(--ink-line)", borderRadius: 4,
                    padding: 6, cursor: "pointer", color: "var(--ink-muted)",
                    display: "flex", alignItems: "center", justifyContent: "center",
                  }}
                >
                  <X size={16} />
              ...
              {(modal.spec.videosExtras?.length ?? 0) > 1 && (
                <div style={{
                  padding: "12px 20px 16px", borderTop: "1px solid var(--ink-line)",
                  display: "flex", flexWrap: "wrap", gap: 8,
                }}>
                  {modal.spec.videosExtras!.map((v) => (
                    <button
                      key={v.id}
                      onClick={() => setModal({ ...modal, videoId: v.id })}
                      style={{
                        fontFamily: "var(--font-worksans)", fontSize: "0.75rem",
