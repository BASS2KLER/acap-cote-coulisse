...
export default function GaleriePage() {
  ...
  return (
    <>
      ...
      {/* Onglets */}
      <div style={{ borderBottom: "1px solid var(--ink-line)", background: "var(--white)", position: "sticky", top: 72, zIndex: 10 }}>
          ...
          {TABS.map((t) => (
            <button
              ...
              style={{
                ...
                background: "none",
                border: "none",
                borderBottom: tab === t.id ? "3px solid var(--brick)" : "3px solid transparent",
                color: tab === t.id ? "var(--brick)" : "var(--ink-muted)",
                cursor: "pointer",
        ...
        <section style={{ ...S, padding: "64px 48px" }}>
          ...
          {!spectacles && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {[1, 2, 3, 4].map((i) => (
                <div key={i} style={{ height: 280, borderRadius: 16, background: "var(--cream-deep)", border: "1px solid var(--sand)" }} />
              ))}
            </div>
          )}
        ...
        <section style={{ ...S, padding: "64px 48px" }}>
          {!galerieItems && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 16 }}>
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div key={i} style={{ aspectRatio: "1", borderRadius: 12, background: "var(--cream-deep)", border: "1px solid var(--sand)" }} />
              ))}
            </div>
          )}
          ...
          {galerieItems && photos.length === 0 && (
            <div style={{
              ...
              Photos à venir — ajoutez-en depuis l'<Link href="/admin/galerie" style={{ color: "var(--brick)" }}>espace admin</Link>.
            </div>
          )}
                      ...
                      {catPhotos.map((photo) => (
                        <div key={photo._id} style={{
                          ...
                          {photo.photoUrl ? (
                            ...
                            <div style={{ aspectRatio: "1", background: "var(--cream-deep)", display: "flex", alignItems: "center", justifyContent: "center" }}>
